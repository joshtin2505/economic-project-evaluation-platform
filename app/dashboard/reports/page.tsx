...
import { Bar, BarChart, Line, LineChart, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceLine } from "recharts";
...
export default function ReportsPage() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      alert('Las librerías de exportación PDF no están instaladas. Usando impresión nativa como alternativa.');
      window.print();
    } finally {
      setIsExporting(null);
    }
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const summaryData = [
        ...
        ['Índice de Rentabilidad', selectedProject.results?.profitabilityIndex || 0],
        ['Período de Recuperación', selectedProject.results?.paybackPeriod || 0],
      ...
      let capitalData: any[][] = [];
      if (selectedProject.tmar_method === 'mixta' && selectedProject.funding_sources) {
        capitalData = [
          ['Fuente de Financiamiento', '% Participación', 'Tasa de Costo', 'Costo Ponderado'],
      ...
      XLSX.writeFile(wb, `Reporte_Proyecto_${selectedProject.name || 'Proyecto'}.xlsx`);
    } catch (error) {
  ...
  const handleExportCSV = () => {
    ...
    try {
      ...
      csv += 'Año,Ingresos Brutos,Egresos/Costos,Valor de Salvamento,Flujo Neto (FNE),FNE Descontado\n';
      ...
      link.setAttribute('download', `Reporte_Proyecto_${selectedProject.name || 'Proyecto'}.csv`);
  ...
  const waterfallData = useMemo(() => {
    ...
    const data = [
      { name: 'Initial Investment', value: -initialInvestment, cumulative: -initialInvestment },
    ];
  ...
  const tirIterations = useMemo(() => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      let derivative = 0;
      detailedCashFlows.forEach((flow) => {
        derivative -= (flow.year * flow.netCashFlow) / Math.pow(1 + rate, flow.year + 1);
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card hidden className="cursor-pointer transition-all hover:border-primary/30 hover:shadow-md print:hidden">
          <CardContent className="flex flex-col items-center py-6 text-center">
            ...
            >
              ...
              {isExporting === 'excel' ? 'Generando...' : t("exports.excel.action")}
            </Button>
          </CardContent>
        </Card>
      ...
      <Card className="print:p-0 print:m-0 print:border-0! print:ring-0! print:w-full ">
        ...
        <CardContent className="print:p-0 print:m-0 print:border-0! print:ring-0! print:w-full">
          {!selectedProject ? (
            ...
            <div id="report-content" className="rounded-lg border print:border-0 bg-background p-8 print:p-0">
              ...
              <div className="mt-6">
                ...
                <div className="mt-4 rounded-lg bg-primary/5 p-4">
                  <p className="text-sm leading-relaxed">
                    {selectedProject.results?.isViable && selectedProject.results?.irr && selectedProject.results?.tmar
              ...
              {/* TMAR Breakdown */}
              {selectedProject.tmar_method === "mixta" && selectedProject.funding_sources && (
                <div className="mt-6">
                  <h2 className="text-lg font-semibold">Desglose de la TMAR (Método Mixto)</h2>
                  <div className="mt-4">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-right">Tasa de Costo</TableHead>
                          <TableHead className="text-right">Costo Ponderado</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {selectedProject.funding_sources.map((source: any, index: number) => {
                          const share = toNumber(source.share);
                          const rate = toNumber(source.rate);
                          const weightedCost = (share / 100) * rate;
                          return (
                            <TableRow key={index}>
                              <TableCell>{source.name || `Fuente ${index + 1}`}</TableCell>
                              ...
                              <TableCell className="text-right font-mono">{weightedCost.toFixed(2)}%</TableCell>
                            </TableRow>
                          );
                        })}
                        <TableRow className="bg-muted/50">
                          <TableCell className="font-semibold">Total</TableCell>
                          <TableCell className="text-right font-semibold font-mono">100.0%</TableCell>
                          <TableCell></TableCell>
              ...
              <div className="mt-6 ">
                <h2 className="text-lg font-semibold">Análisis de Flujo de Caja Detallado</h2>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-right">Egresos/Costos</TableHead>
                        {/* <TableHead className="text-right">Valor de Salvamento</TableHead> */}
                        <TableHead className="text-right">Flujo Neto (FNE)</TableHead>
                    ...
                    <TableBody>
                      {detailedCashFlows.map((flow, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell className="text-right font-mono">${flow.grossInflow.toLocaleString()}</TableCell>
                          <TableCell className="text-right font-mono">${flow.operatingCost.toLocaleString()}</TableCell>
                          {/* <TableCell className="text-right font-mono">${flow.salvageValue.toLocaleString()}</TableCell> */}
                          <TableCell className="text-right font-mono">${flow.netCashFlow.toLocaleString()}</TableCell>
                      ...
                      <TableRow className="bg-muted/50">
                        <TableCell className="font-semibold">Total</TableCell>
                        <TableCell className="text-right font-semibold font-mono">
                          ${detailedCashFlows.reduce((sum, f) => sum + f.grossInflow, 0).toLocaleString()}
                        </TableCell>
                        <TableCell className="text-right font-semibold font-mono">
                          ${detailedCashFlows.reduce((sum, f) => sum + f.operatingCost, 0).toLocaleString()}
                        </TableCell>
                        {/* <TableCell className="text-right font-semibold font-mono">
                          ${detailedCashFlows.reduce((sum, f) => sum + f.salvageValue, 0).toLocaleString()}
                        </TableCell> */}
                        <TableCell className="text-right font-semibold font-mono">
                          ${detailedCashFlows.reduce((sum, f) => sum + f.netCashFlow, 0).toLocaleString()}
                        </TableCell>
                        {/* <TableCell className="text-right font-semibold font-mono">
              ...
              {/* Charts Section */}
              <div className="mt-6 grid print:grid-cols-1 gap-6 sm:grid-cols-2">
                {/* Waterfall Chart */}
                <div>
                  <h2 className="text-lg font-semibold">Gráfico de Cascada (VPN)</h2>
                  <div className="mt-4 h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={waterfallData}>
                        ...
                        <Tooltip 
                          formatter={(value: any) => typeof value === 'number' ? `$${value.toLocaleString()}` : 'N/A'}
                          contentStyle={{ fontSize: '12px' }}
                        />
                        <Bar dataKey="value" fill={waterfallData[0]?.value < 0 ? '#ef4444' : '#22c55e'} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                ...
                <div>
                  <h2 className="text-lg font-semibold">Flujo Acumulado (Payback)</h2>
                  <div className="mt-4 h-64">
                    <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="mt-6">
                <h2 className="text-lg font-semibold">Memoria de Cálculo TIR (Newton-Raphson)</h2>
                <div className="mt-4">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-right">VPN Resultante</TableHead>
                        <TableHead className="text-right">Ajuste (%)</TableHead>
                        <TableHead className="text-right">Nueva Tasa (%)</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {tirIterations.map((iter) => (
                        <TableRow key={iter.iteration}>
                          ...
                          <TableCell className="text-right font-mono">{iter.rate}%</TableCell>
                          <TableCell className="text-right font-mono">${iter.npv}</TableCell>
                          <TableCell className="text-right font-mono">{iter.adjustment}%</TableCell>
                          <TableCell className="text-right font-mono">{iter.newRate}%</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
