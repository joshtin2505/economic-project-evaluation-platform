...
export function useProjectForm(initial?: {
  ...
  const [initialInvestment, setInitialInvestment] = useState<number>(initial?.initial_investment ?? 500000)
  const [periods, setPeriods] = useState<number>(initial?.periods ?? initial?.cashFlows?.length ?? 5)
  const [discountRate, setDiscountRate] = useState<number>(initial?.discount_rate ?? 12)
  const [inflation, setInflation] = useState<number>(initial?.inflation ?? 3)
  const [riskPremium, setRiskPremium] = useState<number>(initial?.risk_premium ?? 2)

  const [cashFlows, setCashFlows] = useState<CashFlowRow[]>(
    initial?.cashFlows ?? Array.from({ length: periods }, (_, i) => ({ period: i + 1, inflow: 0, outflow: 0 }))
  )
  ...
  const setInitialValues = (data: any, flows: CashFlowRow[] | null) => {
    ...
    if (flows && flows.length > 0) {
      setCashFlows(flows.map((f) => ({ period: f.period, inflow: Number(f.inflow), outflow: Number(f.outflow) })))
      setPeriods(flows.length)
    }
  }
